...
import torch
import pdb
from model.pointpillars.ops.iou3d_module import boxes_overlap_bev, boxes_iou_bev
import torchvision
...
def filter_and_keep_bbox(result, pcd_limit_range, score_threshold=0.3, nms_thresh=0.1):
    lidar_bboxes = torch.from_numpy(result['lidar_bboxes'])
    labels = torch.from_numpy(result['labels'])
    scores = torch.from_numpy(result['scores'])

    if torch.is_tensor(lidar_bboxes):
        lidar_bboxes = lidar_bboxes.cpu()
    ...
    bboxes2d = result['bboxes2d']
    camera_bboxes = result['camera_bboxes']

    pcd_limit_range = torch.from_numpy(pcd_limit_range).cpu()
    flag1 = lidar_bboxes[:, :3] > pcd_limit_range[:3]
    flag2 = lidar_bboxes[:, :3] < pcd_limit_range[3:]
    range_mask = torch.all(flag1, dim=-1) & torch.all(flag2, dim=-1)

    score_mask = scores > score_threshold

    combined_mask = range_mask & score_mask

    curr_bboxes = lidar_bboxes[combined_mask]
    curr_labels = labels[combined_mask]
    curr_scores = scores[combined_mask]
    curr_bboxes2d = bboxes2d[combined_mask]
    curr_camera_bboxes = camera_bboxes[combined_mask]

    beva = torch.zeros((curr_bboxes.shape[0], 4))
    beva[:, 0] = curr_bboxes[:, 0] - curr_bboxes[:, 3] / 2
    beva[:, 1] = curr_bboxes[:, 1] - curr_bboxes[:, 4] / 2
    beva[:, 2] = curr_bboxes[:, 0] + curr_bboxes[:, 3] / 2
    beva[:, 3] = curr_bboxes[:, 1] + curr_bboxes[:, 4] / 2

    keep_indices = torchvision.ops.nms(beva, curr_scores, nms_thresh)
    keep_indices = keep_indices.numpy()

    result = {
        'lidar_bboxes': curr_bboxes[keep_indices].numpy(),
        'labels': curr_labels[keep_indices].numpy(),
        'scores': curr_scores[keep_indices].numpy(),
        'bboxes2d': curr_bboxes2d[keep_indices],
        'camera_bboxes': curr_camera_bboxes[keep_indices]
    }
    return result
